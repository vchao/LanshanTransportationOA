...
#import "LTMailSendViewController.h"
...
@interface LTMailSendViewController ()

@end

@implementation LTMailSendViewController
...
- (IBAction)tapSendMail:(id)sender{
    if (revicerField.text.length > 0) {
        if (titleField.text.length > 0) {
            ...
            NSString *URLString = [NSString stringWithFormat:@"%@/maillist.php?act=add&uid=%ld",
                                   API_DOMAIN,uid];
...
-(void)alertWithTable:(UITableView *)tableView
{
    ...
    CGRect pickerFrame = CGRectMake(0, 40, _MainScreen_Width-24, 200);
...
- (void)checkBoxSelect:(id)sender{
    ...
    NSString *indexStr = [NSString stringWithFormat:@"%ld", (long)btn.tag];
    NSArray *array = dic[indexStr];
    for (int i=0; i<[array count]; i++) {
        if (btn.isSelected) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LTContactCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    if (cell == nil) {
        cell = [[LTContactCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
